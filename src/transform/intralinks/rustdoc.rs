...
fn run_rustdoc(
    ...
    let rustdoc_json_path: PathBuf = {
        ...
        const EXPECTED_RUST_TOOLCHAIN: &str = "nightly-2026-01-24";
        let toolchain = match rustup_toolchain::is_installed(EXPECTED_RUST_TOOLCHAIN) {
            Ok(true) => EXPECTED_RUST_TOOLCHAIN,
            // WIP! report this as a warning or something...
            Ok(false) => {
                eprintln!(
                    "Intralink resolution uses unstable rustdoc features. To ensure that intralinks work as expected, you should install this specific nightly toolchain:"
                );
                eprintln!();
                eprintln!("    rustup toolchain install {EXPECTED_RUST_TOOLCHAIN}");
                eprintln!();
                eprintln!("Since it's not install, I'll default to `nightly`.");
                "nightly"
            }
            // WIP! return a proper error.
            Err(e) => {
                eprintln!("Error checking for toolchain with rustup: {e}");
                "nightly"
            }
        };
